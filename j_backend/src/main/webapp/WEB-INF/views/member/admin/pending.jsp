    ...
    <div class="container mt-4">
      ...
      <div class="card">
        <div class="card-header">
          <h5 class="mb-0">
            <i class="bi bi-clock-fill"></i> 승인 대기 중인 회원 목록
          </h5>
        </div>
        <div class="card-body">
          <c:choose>
              ...
              <div class="table-responsive">
                <table class="table table-hover">
                  ...
                  <tbody>
                    <c:forEach
                      var="memberView"
                      items="${pendingMemberViewList}"
                    >
                      <tr>
                        <td><strong>${memberView.member.id}</strong></td>
                        <td>
                          <div class="d-flex align-items-center">
                            <strong>${memberView.member.username}</strong>
                            <c:if test="${memberView.member.role == 'ADMIN'}">
                              <i
                                class="bi bi-shield-fill text-danger ms-1"
                                title="시스템 관리자"
                              ></i>
                            </c:if>
                          </div>
                        </td>
                        <td>${memberView.member.nickname}</td>
                        <td>
                          <small class="text-muted"
                            >${memberView.member.email}</small
                          >
                        </td>
                        <td>
                          <small>
                            <c:choose>
                              <c:when
                                test="${not empty memberView.createDateFormatted}"
                              >
                                <fmt:formatDate
                                  value="${memberView.createDateFormatted}"
                        ...
                        <td>
                          <span class="badge bg-warning text-dark"
                            >${memberView.member.status.description}</span
                          >
                        </td>
                        <td>
                          <div class="btn-group btn-group-sm" role="group">
                            <form
                              method="post"
                              action="/members/admin/${memberView.member.id}/approve"
                              class="d-inline"
                            >
                              <button
                                type="submit"
                            ...
                            <form
                              method="post"
                              action="/members/admin/${memberView.member.id}/reject"
                              class="d-inline"
                            >
                              <button
                                type="submit"
                    ...
                    <ul class="pagination justify-content-center mb-0">
                      <c:if test="${members.hasPrevious()}">
                        <li class="page-item">
                          <a
                            class="page-link"
                            href="?page=${members.number - 1}&size=${members.size}"
                          >
                      ...
                      <c:forEach
                        var="i"
                        begin="0"
                        end="${members.totalPages - 1}"
                      >
                        <c:if
                          test="${i >= members.number - 2 && i <= members.number + 2}"
                        >
                          <li
                            class="page-item ${i == members.number ? 'active' : ''}"
                          >
                            <a
                              class="page-link"
                              href="?page=${i}&size=${members.size}"
                              >${i + 1}</a
                            >
                          </li>
                        </c:if>
