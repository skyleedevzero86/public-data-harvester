...
public class MemberCacheService {
    ...
    private static final Logger log = LoggerFactory.getLogger(MemberCacheService.class);
    ...
    @Autowired
    public MemberCacheService(@Qualifier("antockRedisTemplate") RedisTemplate<String, Object> redisTemplate,
    ...
    public void init() {
        ...
        if (cacheAvailable) {
            log.info("MemberCacheService 초기화 완료 - TTL: {}분", cacheTtlMinutes);
        } else {
            log.warn("MemberCacheService 캐시 비활성화 - 직접 DB 조회로 동작");
        }
    }
    ...
    public void cacheMember(Member member) {
        ...
        try {
            ...
            log.debug(" 회원 정보 캐시 저장 - ID: {}, TTL: {}분",
                    member.getId(), cacheTtlMinutes);
    ...
    public void cacheMemberResponse(MemberResponse memberResponse) {
        ...
        try {
            ...
            log.debug(" 회원 응답 캐시 저장 - ID: {}, username: {}",
                    memberResponse.getId(), memberResponse.getUsername());
    ...
    public void evictMemberCache(Long memberId) {
        ...
        try {
            ...
            log.info(" 회원 캐시 무효화 - ID: {}, 회원정보: {}, 프로필: {}",
                    memberId, memberDeleted, profileDeleted);
    ...
    public void evictAllMemberCache() {
        ...
        try {
            ...
            log.warn(" 전체 회원 캐시 무효화 - 회원정보: {}, 프로필: {}",
                    memberDeleted, profileDeleted);
    ...
    public void checkCacheHealth() {
        ...
        if (!wasAvailable && cacheAvailable) {
            log.info(" Member 캐시 연결 복구됨");
        } else if (wasAvailable && !cacheAvailable) {
            log.warn(" Member 캐시 연결 실패 감지");
        }
    }
    ...
    public void logCacheStatistics() {
        ...
        if (stats.getTotalRequests() > 0) {
            log.info(" Member 캐시 통계 - 히트율: {:.1f}%, 히트: {}, 미스: {}, 오류: {}",
    ...
    private boolean checkCacheConnection() {
        ...
        try {
            ...
            log.debug(" Member 캐시 헬스체크 실패 - {}", e.getMessage());
            return false;
        }
    }
    ...
    private void handleCacheError(String operation, Exception e) {
        ...
        log.error(" Member 캐시 {} - {}", operation, e.getMessage());

        if (cacheErrors.get() % 10 == 0) {
            log.warn("️ Member 캐시 오류 빈발 - 총 {}회", cacheErrors.get());
        }
    }
