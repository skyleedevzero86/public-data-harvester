...
-- 사업자명 전문검색용 인덱스 (기본 영어 설정 사용)
CREATE INDEX IF NOT EXISTS idx_corp_mast_biz_nm_gin ON corp_mast USING gin(to_tsvector('english', biz_nm)); 
...
SELECT 
    m.id,
    '$2a$10$' || substr(md5((m.id::text || i::text || random()::text)::text), 1, 53),
